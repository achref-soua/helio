...
- ✅ **Bring your own churn model** — upload an ONNX or XGBoost model (pickle refused by design) or connect your own model server; validated in a locked-down sandbox, one click to activate, automatic fallback to the built-in model if yours ever fails — plus a one-click training-data CSV
