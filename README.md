...
- ✅ **Per-element color palettes** — every published surface sets its own background, text, button, and accent colors; new surfaces are seeded from your brand color and each value is re-validated hex-only on the server before it reaches a hosted page or the SDK (hosted forms, landing pages, and in-app messages; on-site widgets follow)
