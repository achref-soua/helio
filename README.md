...
- ✅ **Per-element color palettes** — every published surface sets its own background, text, button, and accent colors; new surfaces are seeded from your brand color and each value is re-validated hex-only on the server before it reaches a hosted page (hosted forms and landing pages; in-app messages and on-site widgets follow)
